...
var server_info = {
	name = "DBreak",      # Holds the name of the server
	max_players = 200,      # Maximum allowed connections
	used_port = 3306        # Listening port
}
...
func create_server():
	...
	register_player($"/root/Gamestate".player_info)
...
func _on_connected_to_server():
	...
	$"/root/Gamestate".player_info.net_id = get_tree().get_network_unique_id()
	# Request the server to register this new player across all connected players
	rpc_id(1, "register_player", $"/root/Gamestate".player_info)
...
func _on_disconnected_from_server():
	...
	$"/root/Gamestate".player_info.net_id = 1
