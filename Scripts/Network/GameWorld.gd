...
func _ready():
	...
	$"/root/Network".connect("player_list_changed", self, "_on_player_list_changed")
	# If we are in the server, connect to the event that will deal with player despawning
	if (get_tree().is_network_server()):
		$"/root/Network".connect("player_removed", self, "_on_player_removed")
	
#	# Update the lblLocalPlayer label widget to display the local player name
#	$HUD/PanelPlayerList/lblLocalPlayer.text = $"/root/Gamestate".player_info.name
	
	# Spawn the players
		rpc_id(1, "spawn_players", $"/root/Gamestate".player_info, -1)

remote func despawn_player(pinfo):
	if (get_tree().is_network_server()):
		for id in $"/root/Network".players:
...
#	for p in $"/root/Network".players:
#		if (p != $"/root/Gamestate".player_info.net_id):
#			var nlabel = Label.new()
#			nlabel.text = $"/root/Network".players[p].name
...
remote func spawn_players(pinfo, spawn_index):
	...
	if (spawn_index == -1):
		spawn_index = $"/root/Network".players.size()
	...
	if (get_tree().is_network_server() && pinfo.net_id != 1):
		...
		for id in $"/root/Network".players:
			...
			if (id != pinfo.net_id):
				rpc_id(pinfo.net_id, "spawn_players", $"/root/Network".players[id], s_index)
